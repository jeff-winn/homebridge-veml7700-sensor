...
export class Veml7700Accessory implements AccessoryPlugin {
    ...
    public constructor(private log: Logging, c: AccessoryConfig, private api: API) {
        ...
        this.accessoryInformation = new AccessoryInformationImpl(this, this.api);
        this.accessoryInformation.init();

        this.rainSensor = new RainSensorImpl(this, this.api);
    ...
    public getServices(): Service[] {
        ...
        const accessoryInformation = this.accessoryInformation.getUnderlyingService();
        if (accessoryInformation !== undefined) {
            result.push(accessoryInformation);
        }

        const rainSensor = this.rainSensor.getUnderlyingService();
        if (rainSensor !== undefined) {
            result.push(rainSensor);
        }
